...
import { Image, Pressable, StyleProp, StyleSheet, TextStyle, View } from "react-native";
...
export const CardItem = (props: CardItemProps) => {
	...
	return (
		<Pressable style={styles.container}>
			<Image source={CardCover} style={[ StyleSheet.absoluteFill, { zIndex: -1 } ]} />

			<Icon style={styles.icon} />

			<View style={styles.dataContainer}>
				<View>
					<Text style={[styles.moneyText, {color: Colors.dark.foreground}]}>{formatUSD(data.balance)}</Text>
				</View>

				<View style={styles.cardInfo}>
					<Text style={[styles.cardInfoText, {color: Colors.dark.foreground}]}>{title}</Text>

					<Text style={[styles.cardInfoText, {color: Colors.dark.foreground}]}>{safeCardNumber}</Text>					
				</View>
			</View>
		</Pressable>
	)
