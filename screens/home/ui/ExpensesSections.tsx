...
import { convertExpenseTypeToColor, Expense, formatExpenseType } from "@/entities/expense";
...
export const ExpensesSections = (props: ExpansesSections) => {
	...
	return (
		<View style={styles.container}>
			{days.map(day => <DaySection key={day.date.getTime()} date={day.date} expanses={day.expanses} /> )}
		</View>
	)
}
	...
	return (
		<View style={styles.daySectionContainer}>
			...
			<View style={styles.daySectionItemsContainer}>
				{expanses.map(expanse => <SectionItem key={expanse.date + expanse.recipient.name} data={expanse} />)}
			</View>
		</View>
	)
}
...
const SectionItem = (props: SectionItemProps) => {
	...
	return (
		<View style={[ styles.sectionItem, { backgroundColor: Colors[colorSchema ?? 'light'].container } ]}>
			...
			<View style={styles.infoContainer}>
				...
				<View style={styles.row}>
					<View style={[ styles.typeCirle, { backgroundColor: convertExpenseTypeToColor(data.type) } ]} />
				
					<View>
						<Text style={[ styles.typeName, { color: Colors[colorSchema ?? 'light'].extra } ]}>{formatExpenseType(data.type)}</Text>
					</View>
				</View>
			</View>
			...
			<View style={[ styles.infoContainer, { marginLeft: 'auto' } ]}>
				...
				<Text style={[ styles.date, { color: Colors[colorSchema ?? 'light'].extra } ]}>{formatDate(data.date)}</Text>
			</View>
		</View>
	)
}
...
function formatSectionDate(date: Date): string {
	...
	return date.toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' }); // "14 September 2024"
}
