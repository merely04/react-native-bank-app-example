...
export const NotificationSection = (props: NotificationSectionProps) => {
	...
	return (
		<View style={[ styles.notificationSectionContainer, { borderBottomColor: Colors[colorScheme ?? 'light'].border } ]}>
			<View style={styles.notificationSectionDateContainer}>
				<Text style={[styles.notificationSectionDateText]}>{formatDate(date)}</Text>
			</View>
	...
	return (
		<View style={styles.notificationItemContainer}>
			{data.paymentInfo !== null && data.paymentInfo.amount > 0 ? (
				<Image source={{ uri: data.paymentInfo.sender.avatarUrl }} style={styles.notificationItemIconContainer} />
			) : Icon === null ? null : (
				<View style={[ styles.notificationItemIconContainer, { backgroundColor: Colors[colorScheme ?? 'light'].container} ]}>
					<Icon style={styles.notificationItemIcon} color={Colors[colorScheme ?? 'light'].tint} />
				</View>
			)}
			...
			<View style={styles.notificationItemInfoContainer}>
				...
				{data.paymentInfo !== null && (
					<View style={{ marginTop: -2 }}>
						<Text style={[ styles.notificationItemInfoAmount, { color: Colors[colorScheme ?? 'light'].tint } ]}>
				...
				<View>
					<Text style={styles.notificationItemInfoExtra}>{formatCreatedAt(data.createdAt)} · {formatNotificationType(data.type)}</Text>
				</View>
			</View>

			{!data.isRead && (
				<View style={styles.notificationItemStatusContainer}>
					<View style={[ styles.notificationItemStatus, { backgroundColor: Colors[colorScheme ?? 'light'].tint } ]} />
				</View>
			)}
		</View>
	)
